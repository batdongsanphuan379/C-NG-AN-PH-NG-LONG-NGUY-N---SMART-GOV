...
const StatCard = ({ label, value, icon: Icon, color, trend, isExporting }: any) => (
  <motion.div 
    ...
    className="bg-white p-5 md:p-6 rounded-3xl md:rounded-[2rem] shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-md transition-shadow"
  >
...
export default function AdminDashboard() {
  ...
  return (
    <div ref={dashboardRef} className={`space-y-6 md:space-y-8 pb-12 ${isExporting ? '' : 'animate-in fade-in duration-500'} bg-[#F8FAFC] p-2 md:p-4 rounded-3xl md:rounded-[2.5rem]`}>
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-6">
        ...
        <StatCard label="Xác nhận" value={stats.confirmed} icon={Calendar} color="bg-indigo-500" trend={8} isExporting={isExporting} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        {/* Charts Section */}
        <div className="lg:col-span-2 bg-white p-5 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <div>
              <h3 className="text-lg md:text-xl font-bold text-gray-800">Biểu đồ Lượng khách</h3>
              <p className="text-xs md:text-sm text-gray-500">Phân bổ theo khung giờ</p>
            </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col items-center">
            <h3 className="text-lg md:text-xl font-bold text-gray-800 self-start mb-1 md:mb-2">Trạng thái Hồ sơ</h3>
            <p className="text-xs md:text-sm text-gray-500 self-start mb-6 md:mb-8">Tỷ lệ xử lý thực tế</p>
      ...
      <div className="bg-gradient-to-r from-[#1A5FB4] to-[#144A8E] p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-blue-500/20">
         <div className="space-y-2 text-center md:text-left">
            <h3 className="text-xl md:text-2xl font-bold flex items-center justify-center md:justify-start gap-2">
              <TrendingUp />
              Hiệu suất hoạt động
            </h3>
            <p className="text-blue-100 text-xs md:text-sm max-w-lg leading-relaxed">
              Tỷ lệ khách đến đúng hẹn tăng cao so với trước. 
         ...
         <button 
           ...
           className={`w-full md:w-auto bg-white text-[#1A5FB4] export-ignore px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-wider shadow-lg hover:bg-blue-50 transition-all active:scale-95 whitespace-nowrap ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}`}
         >
